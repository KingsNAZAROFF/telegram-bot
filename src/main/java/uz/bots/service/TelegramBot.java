...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    public static final Map<Long, Admin> admins = new HashMap<>();
//        private static final Long[] ADMINS = {184535535L, 602701577L, 565200855L, 2004778336L, 1577592359L};
    private static final Long[] ADMINS = {602701577L};
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (!isAdmin) {
                ...
                case "getPartTime":
                    try {
                        if (!message.getText().equals("Ortga qaytish")) {
                            ...
                            try {
                                String query = "INSERT INTO tickets(user_id,ticket_name,ticket_time) VALUES ";
                                query+= "(" + users.get(userId).getTgId()+ ","+ "'" + users.get(userId).getBiletName() +"'," + users.get(userId).getDatePart()+")";
                                System.out.println(query);;
                                Ticket.statement.execute(query);
            ...
            if (admins.get(message.getFrom().getId()) == null) {
                admins.put(message.getFrom().getId(), new Admin());
            }
            adminSender(message, new AdminService().adminMenu(message));
        }
    }
    ...
    public void adminSender(Message message, String text) {
        ...
        AdminKeyboard.userKeyboard(sendMessage, admins.get(message.getFrom().getId()));
        try {
            execute(sendMessage);
